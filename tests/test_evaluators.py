...
async def test_delete(client: Humanloop, async_client: AsyncHumanloop) -> None:
    ...
    assert (
        client.evaluators.delete(id="ev_890bcd")  # type: ignore[func-returns-value]
        is None
    )

    assert (
        await async_client.evaluators.delete(id="ev_890bcd")  # type: ignore[func-returns-value]
        is None
    )
...
async def test_remove_deployment(client: Humanloop, async_client: AsyncHumanloop) -> None:
    # Type ignore to avoid mypy complaining about the function not being meant to return a value
    assert (
        client.evaluators.remove_deployment(id="ev_890bcd", environment_id="staging")  # type: ignore[func-returns-value]
        is None
    )

    assert (
        await async_client.evaluators.remove_deployment(id="ev_890bcd", environment_id="staging")  # type: ignore[func-returns-value]
        is None
    )
