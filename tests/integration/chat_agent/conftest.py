...
from tests.conftest import APIKeys
...
@pytest.fixture()
def surfer_agent_scenario_factory(
    humanloop_client: Humanloop,
    get_test_path: Callable[[str], str],
    api_keys: APIKeys,
) -> Callable[[bool], SurferAgentScenario]:
    def factory(use_overload_call: bool) -> SurferAgentScenario:
        import json
        import random
        ...
        def call_agent(messages: list[dict[str, Any]]) -> str:  # type: ignore [call-arg]
            if use_overload_call:
                output = humanloop_client.prompts.call(
                    path=get_test_path("Agent Prompt"),
                    messages=messages,  # type: ignore [arg-type]
                    prompt={  # type: ignore [arg-type]
                        "model": "gpt-4o-mini",
                        "tools": [
                            # TODO: json_schema property should specify
                            # an OpenAI/ sHL format or be aware of the context
                            # it's called from; note the difference between
                            # prompts.call and openai.chat.completions.create
                            calculator.json_schema,
                            pick_random_number.json_schema,
                        ],
                        **LLM_HYPERPARAMETERS,  # type: ignore
                    },
                )

                if output.logs[0].output_message.tool_calls:  # type: ignore [union-attr]
                    for tool_call in output.logs[0].output_message.tool_calls:  # type: ignore [union-attr]
                        arguments = json.loads(tool_call.function.arguments)  # type: ignore [arg-type]
                ...
                return output.logs[0].output_message.content  # type: ignore
            ...
            if output.choices[0].message.tool_calls:  # type: ignore [attr-defined]
                for tool_call in output.choices[0].message.tool_calls:  # type: ignore [attr-defined]
            ...
            return output.choices[0].message.content  # type: ignore [attr-defined]
