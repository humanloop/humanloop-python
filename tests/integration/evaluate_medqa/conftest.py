...
@pytest.fixture()
def evaluate_medqa_scenario_factory(
    humanloop_client: "Humanloop",
    get_test_path: Callable[[str], str],
    api_keys: APIKeys,
    medqa_knowledge_base_path: str,
    medqa_dataset_path: str,
) -> Callable[[bool], MedQAScenario]:
    def factory(use_overload_call: bool) -> MedQAScenario:
        import inspect
        ...
        def call_model(**inputs):
            ...
            if use_overload_call:
                chat_completion = humanloop_client.prompts.call(
                    path=get_test_path("Call Model"),
                    prompt={
                        "model": MODEL,
                        "temperature": 0,
                        "presence_penalty": 0,
                ...
                return chat_completion.logs[0].output_message.content
            else:
