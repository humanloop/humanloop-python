...
async def test_delete(client: Humanloop, async_client: AsyncHumanloop) -> None:
    ...
    assert (
        client.datasets.delete(id="id")  # type: ignore[func-returns-value]
        is None
    )

    assert (
        await async_client.datasets.delete(id="id")  # type: ignore[func-returns-value]
        is None
    )
...
async def test_remove_deployment(client: Humanloop, async_client: AsyncHumanloop) -> None:
    # Type ignore to avoid mypy complaining about the function not being meant to return a value
    assert (
        client.datasets.remove_deployment(id="ds_b0baF1ca7652", environment_id="staging")  # type: ignore[func-returns-value]
        is None
    )

    assert (
        await async_client.datasets.remove_deployment(id="ds_b0baF1ca7652", environment_id="staging")  # type: ignore[func-returns-value]
        is None
    )
