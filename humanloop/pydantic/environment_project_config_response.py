# coding: utf-8

"""
    Humanloop API

    The Humanloop API allows you to interact with Humanloop from your product or service.  You can do this through HTTP requests from any language or via our official Python or TypeScript SDK.  To install the official [Python SDK](https://pypi.org/project/humanloop/), run the following command:  ```bash pip install humanloop ```  To install the official [TypeScript SDK](https://www.npmjs.com/package/humanloop), run the following command:  ```bash npm i humanloop ```  ---  Guides and further details about key concepts can be found in [our docs](https://docs.humanloop.com/).

    The version of the OpenAPI document: 4.0.1
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal
from pydantic import BaseModel, Field, RootModel


class EnvironmentProjectConfigResponse(BaseModel):
    # String ID of project the model config belongs to. Starts with `pr_`.
    project_id: str = Field(alias='project_id')

    # Name of the project the model config belongs to.
    project_name: str = Field(alias='project_name')

    # The ID of the environment.
    environment_id: str = Field(alias='environment_id')

    # The name of the environment.
    environment_name: str = Field(alias='environment_name')

    # Tag for the environment e.g. 'default' or 'other'.
    environment_tag: str = Field(alias='environment_tag')

    # Model config unique identifier generated by Humanloop.
    model_config_id_: str = Field(None, alias='model_config_id')

    # A friendly display name for the model config.
    model_config_name_: str = Field(None, alias='model_config_name')

    # String ID of experiment. Starts with `exp_`.
    experiment_id: str = Field(None, alias='experiment_id')

    # Display name of experiment.
    experiment_name: str = Field(None, alias='experiment_name')
